...
if (login_status_code1 == 200) {
    ...
    if (AdminResetPassword_statusCode == 200) {
		...
		login_response2 = WS.sendRequest(findTestObject('Login', [('email') : email, ('pass') : oldPwd]))
		...
		if (login_status_code2 == 500){
			...
			String error = result2.error
			   ...
			   KeywordUtil.markPassed("Incorrect username or password.")
			 
			   }
			
			   KeywordUtil.markPassed("NewLogin:User tried to login with old password")
//			   WS.verifyResponseStatusCode(login_response2, 200, FailureHandling.STOP_ON_FAILURE)
		
		} else if (login_status_code2 == 200){
		
		KeywordUtil.markFailed("NewLogin:Expected status code is '500' but actual status code is "+login_status_code2)
