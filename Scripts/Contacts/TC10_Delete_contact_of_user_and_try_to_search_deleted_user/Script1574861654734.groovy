...
//        GlobalVariable.ContactID = (test[0])
        ...
        if (delete_contact_status_code == 204) {
            ...
            if (get_contact_status_code2 == 200) {
                'Get contact response'
                KeywordUtil.logInfo('Get Contact Response: ' + get_contact_response2.getResponseText())
                ...
                String[] test2 = get_contact_result2.id

                for (int i = 0; i < test2.length; i++) {
                    println(test2[i])

                    'Check wheather deleted contact present in contact list or not'
                    if (!(test2[i].equals(GlobalVariable.ContactID))) {
                        KeywordUtil.markPassed('Following contact ID not present at the current contact listing. ' + GlobalVariable.ContactID)
						break
                    } else {
                        KeywordUtil.markFailed('Following contact ID present at the current contact listing. ' + GlobalVariable.ContactID)
                    }
                }
            } ///////////////////////////////////////////////////
            else {
                KeywordUtil.markFailed('get_contact_status_code2:Expected status code is \'200\' but actual status code is ' + 
                    get_contact_status_code2)
            }
        } else {
