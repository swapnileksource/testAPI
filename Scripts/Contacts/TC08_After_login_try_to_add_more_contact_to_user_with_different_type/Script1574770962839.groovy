...
if (login_staus_code == 200) {
    ...
    if (add_contact_status_code == 201) {
		...
		////////////////////////////////////////
		'Send Add Contact POST request'
		def add_contact_response2 = WS.sendRequest(findTestObject('Contacts/AddContact', [('value') : GlobalVariable.value, ('type') : GlobalVariable.type]))
	
		'Log add contact response'
		KeywordUtil.logInfo('Add Contact Response2: ' + add_contact_response2.getResponseText())
	
		'Print add contact response'
		println('Add Contact Response2: ' + add_contact_response2.getResponseBodyContent())
	
		'Get add contact status code from the response'
		def add_contact_status_code2 = add_contact_response2.getStatusCode()
	
		'Log add contact status code'
		KeywordUtil.logInfo('Add Contact Status Code2: ' + add_contact_status_code2)
	
		'Print add contact status code'
		println('Add Contact Status Code2: ' + add_contact_status_code2)
	
		'Check add contact status code for 201'
		if (add_contact_status_code2 == 201) {
			KeywordUtil.markPassed('Contact added successfully')
		} else {
			KeywordUtil.markFailed('add_contact_status_code2:Expected status code is \'201\' but actual status code is ' + add_contact_status_code2)
		}
        ...
        KeywordUtil.markFailed('add_contact_status_code:Expected status code is \'201\' but actual status code is ' + add_contact_status_code)
    }
